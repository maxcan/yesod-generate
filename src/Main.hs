...
addModelToModelsFile 
  ...
  -> EntityDef 
  -> Bool -> ErrT ()
addModelToModelsFile defFp hsFp cmd ed useJson = do
  flds <- mapM persistFieldDefToFieldDesc $ entityFields ed
  (gitRev, dirty) <- qRunIO (maybe (error "NO GIT REPO DATA AVAILABLE") return =<< getRevision)
  appendLineToFile defFp $ "  -- generated by yesod-generate " ++ gitRev
  when dirty $ appendLineToFile "  -- BUILT ON A DIRTY REPO.  BE VERY CAREFUL.  Please report to i@cantor.mx"
  appendLineToFile defFp $ "  -- using command: " ++ cmd
